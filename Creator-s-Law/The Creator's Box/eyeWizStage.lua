	...
	function Backgroundss()
		...
		local function UIs ()
			...
			local function adding()
				pandorasBoxFiller = timer.performWithDelay(1000, fillUpCreatorsBox, 0)
...
local function theAttacks()
	...
	eyesMovingLoop = timer.performWithDelay(1000, transitioningEye,0)
    ------------------------------------------------------------------------------------------------------
	local function attack1()
		...
		local function wheelJump (event)
			print("hi")
			local phase = event.phase
			--
			if ( event.phase == "ended" and hero.touchFloor==true) then
			-- Jump procedure here
				  
				  hero.rotation = 0
				  --[[local vx, vy = hero:getLinearVelocity()
				   hero:setLinearVelocity( 0, 0 )
					hero:applyLinearImpulse( nil, -800, hero.x, hero.y )
					]]--
					local function stayUp()
						transition.to(hero, {time = 250, y = hero.y})
					end

					transition.to(hero, {time = 200, y = hero.y - 100, onComplete = stayUp})
					hero.touchFloor =false
					print("ho")
		...
		rocketTime = timer.performWithDelay( 1000,shoot,18)
	end
	------------------------------------------------------------------------------------------------------
	local function attack2()
		...
		horizontalWheelTime = timer.performWithDelay( 500,shoot,32)	
	end
	------------------------------------------------------------------------------------------------------
	local function attack3()
		...
		verticalWheelTime = timer.performWithDelay( 1000,shoot,16)	
	end
	------------------------------------------------------------------------------------------------------
	local function pickAttack ()
		whichAttack = math.random(4,4)
